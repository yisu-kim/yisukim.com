---
title: next-auth 오픈 소스에 기여하기
description: OAuth 프로토콜을 따르게 되면 사용자는 한 서비스(Provider)에서 다른 서비스(Consumer)로 자신이 공유하길 원하는 정보를 안전하게 넘겨줄 수 있게 된다. OAuth의 대표적인 케이스인 소셜 로그인 구현을 돕는 NextAuth를 알아보자.
date: 2022-05-01
...
![추상적인 프로토콜 흐름](/posts/contribute-to-next-auth/추상적인_프로토콜_흐름.png)
...
![소셜 로그인 및 권한 부여](/posts/contribute-to-next-auth/소셜_로그인_및_권한_부여.png)
...
![콜백 문서 수정 코드리뷰](/posts/contribute-to-next-auth/콜백_문서_수정_코드리뷰.png)
...
![카카오 reference 에러 코드 수정 코드리뷰](/posts/contribute-to-next-auth/카카오_reference_에러_코드_수정_코드리뷰.png)
